...
class Trainer(object):
    def __init__(self, hparams):
        ...
        self.opt_g = torch.optim.Adam(self.model.generator.parameters(), lr=0.00002, betas=(0.5, 0.99))
    ...
    def train_step(self, batch, train_gen):
        ...
        grad_enc, _ = self.model.discriminator(x_enc1, x_enc2, None)
        ...
        _, d_features_real = self.model.discriminator(nodes, adj, None)
        _, d_features_fake_enc = self.model.discriminator(nodes_fake_enc, adj_fake_enc, None)

        e_loss_kld = kld_loss(mu, log_var)

        batch_size = d_features_real.size(0)
        norm_factor = torch.triu(((d_features_real.unsqueeze(0) - d_features_real.unsqueeze(1)) ** 2).mean(dim=-1)).sum() / ((batch_size*batch_size-1) / 2)
        e_loss_rec = mse_loss(d_features_fake_enc, d_features_real) / norm_factor
        ...
        if train_gen:
            ...
            _, d_features_real = self.model.discriminator(nodes, adj, None)
            d_logits_fake_enc, d_features_fake_enc = self.model.discriminator(nodes_fake_enc, adj_fake_enc, None)
            d_logits_fake_pri, _ = self.model.discriminator(nodes_fake_pri, adj_fake_pri, None)
