import torch
from torch.nn import Linear, Sequential, ELU, BatchNorm1d, ReLU


class FNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers, non_linearity="relu", batch_norm=False,
                 flatten_for_batch_norm=False, dropout=None, debug=False, batch_norm_in=False, batch_norm_out=False,
                 non_lin_in=False, non_lin_out=False):
        super().__init__()
        self.num_layers = num_layers
        self.batch_norm = batch_norm
        self.flatten_for_batch_norm = flatten_for_batch_norm
        layers = [Linear(input_dim, hidden_dim)]
        layers += [Linear(hidden_dim, hidden_dim) for _ in range(num_layers - 2)]
        layers += [Linear(hidden_dim, output_dim)]
        self.layers = torch.nn.ModuleList(layers)
        self.debug = debug
        if batch_norm:
            #bn_layers = [BatchNorm1d(input_dim)]
            bn_layers = [BatchNorm1d(hidden_dim) for _ in range(num_layers - 1)]
            self.bn_layers = torch.nn.ModuleList(bn_layers)
        if batch_norm_in:
            self.bn_in = BatchNorm1d(input_dim)
        if batch_norm_out:
            self.bn_out = BatchNorm1d(output_dim)
        self.batch_norm_in = batch_norm_in
        self.batch_norm_out = batch_norm_out
        self.non_lin_in = non_lin_in
        self.non_lin_out = non_lin_out
        if non_linearity == "relu":
            self.non_linearity = torch.nn.ReLU()
        elif non_linearity == "elu":
            self.non_linearity = torch.nn.ELU()
        elif non_linearity == "lrelu":
            self.non_linearity = torch.nn.LeakyReLU()
        self.dropout = dropout


    def forward(self, x):
        if self.debug:
            print(x.shape)
        if self.batch_norm_in:
            if self.flatten_for_batch_norm:
                shape = x.shape
                x = x.view(shape[0] * shape[1], shape[2])
                x = self.bn_in(x)
                x = x.view(shape[0], shape[1], shape[2])
            else:
                x = self.bn_in(x)
        if self.non_lin_in:
            x = self.non_linearity(x)
        for i in range(self.num_layers):
            if i > 0:
                if self.batch_norm:
                    if self.flatten_for_batch_norm:
                        shape = x.shape
                        x = x.view(shape[0] * shape[1], shape[2])
                        x = self.bn_layers[i - 1](x)
                        x = x.view(shape[0], shape[1], shape[2])
                    else:
                        x = self.bn_layers[i - 1](x)
                x = self.non_linearity(x)
                if self.dropout is not None:
                    x = torch.nn.functional.dropout(x, p=self.dropout)
            x = self.layers[i](x)
        if self.batch_norm_out:
            if self.flatten_for_batch_norm:
                shape = x.shape
                x = x.view(shape[0] * shape[1], shape[2])
                x = self.bn_out(x)
                x = x.view(shape[0], shape[1], shape[2])
            else:
                x = self.bn_out(x)
        if self.non_lin_out:
            x = self.non_linearity(x)
        return x
